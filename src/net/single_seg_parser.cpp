...
std::pair<size_t, size_t> single_seg_parser::operator()(const bsv::span<const uint8_t> s)
{
    segment_.insert(segment_.end(), s.begin(), s.end());
    return std::make_pair(s.size(), 0);
};

size_t single_seg_parser::read(const size_t read_pos, bsv::span<uint8_t> s)
{
