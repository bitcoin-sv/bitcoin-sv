...
static std::unique_ptr<CBlockIndex> GetBlockIndex(CBlockIndex *pindexPrev, int64_t nTimeInterval,
                                 uint32_t nBits) {
...
BOOST_AUTO_TEST_CASE(retargeting_test) {
    ...
    std::vector<std::unique_ptr<CBlockIndex>> blocks(115);
    ...
    // Genesis block.
    blocks[0] = std::make_unique<CBlockIndex>();
    blocks[0]->nHeight = 0;
    ...
    for (size_t i = 1; i < 100; i++) {
        blocks[i] = GetBlockIndex(blocks[i - 1].get(), params.nPowTargetSpacing,
                                  initialBits);
    }
    ...
    for (size_t i = 100; i < 110; i++) {
        blocks[i] = GetBlockIndex(blocks[i - 1].get(), 2 * 3600, initialBits);
        BOOST_CHECK_EQUAL(
            GetNextWorkRequired(blocks[i].get(), &blkHeaderDummy, config),
            initialBits);
    }

    // Now we expect the difficulty to decrease.
    blocks[110] = GetBlockIndex(blocks[109].get(), 2 * 3600, initialBits);
    currentPow.SetCompact(currentPow.GetCompact());
    currentPow += (currentPow >> 2);
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(blocks[110].get(), &blkHeaderDummy, config),
    ...
    currentPow += (currentPow >> 2);
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(blocks[111].get(), &blkHeaderDummy, config),
        currentPow.GetCompact());
    ...
    currentPow += (currentPow >> 2);
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(blocks[112].get(), &blkHeaderDummy, config),
        currentPow.GetCompact());
    ...
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(blocks[113].get(), &blkHeaderDummy, config),
        powLimit.GetCompact());

    // Once we reached the minimal difficulty, we stick with it.
    blocks[114] = GetBlockIndex(blocks[113].get(), 2 * 3600, powLimit.GetCompact());
    BOOST_CHECK(powLimit.GetCompact() != currentPow.GetCompact());
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(blocks[114].get(), &blkHeaderDummy, config),
        powLimit.GetCompact());
}
...
BOOST_AUTO_TEST_CASE(cash_difficulty_test) {
    ...
    std::vector<std::unique_ptr<CBlockIndex>> blocks(3000);
    ...
    CBlockHeader blkHeaderDummy;
    uint32_t nBits =
        GetNextCashWorkRequired(blocks[2049].get(), &blkHeaderDummy, config);

    // Difficulty stays the same as long as we produce a block every 10 mins.
    for (size_t j = 0; j < 10; i++, j++) {
        blocks[i] = GetBlockIndex(blocks[i - 1].get(), 600, nBits);
        BOOST_CHECK_EQUAL(
            GetNextCashWorkRequired(blocks[i].get(), &blkHeaderDummy, config),
            nBits);
    }
    ...
    BOOST_CHECK_EQUAL(
        GetNextCashWorkRequired(blocks[i++].get(), &blkHeaderDummy, config), nBits);
    ...
    // impact.
    blocks[i] = GetBlockIndex(blocks[i - 1].get(), 6000, nBits);
    nBits = GetNextCashWorkRequired(blocks[i++].get(), &blkHeaderDummy, config);
