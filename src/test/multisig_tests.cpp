...
BOOST_AUTO_TEST_CASE(multisig_IsStandard) {
    ...
    a_and_b << OP_2 << ToByteVector(key[0].GetPubKey())
            << ToByteVector(key[1].GetPubKey()) << OP_2 << OP_CHECKMULTISIG;
    BOOST_CHECK(::IsStandardOutput(config.GetConfigScriptPolicy(), a_and_b, 1, whichType));

    CScript a_or_b;
    a_or_b << OP_1 << ToByteVector(key[0].GetPubKey())
           << ToByteVector(key[1].GetPubKey()) << OP_2 << OP_CHECKMULTISIG;
    BOOST_CHECK(::IsStandardOutput(config.GetConfigScriptPolicy(), a_or_b, 1, whichType));

    CScript escrow;
    escrow << OP_2 << ToByteVector(key[0].GetPubKey())
           << ToByteVector(key[1].GetPubKey())
           << ToByteVector(key[2].GetPubKey()) << OP_3 << OP_CHECKMULTISIG;
    BOOST_CHECK(::IsStandardOutput(config.GetConfigScriptPolicy(), escrow, 1, whichType));
    ...
    BOOST_CHECK(!::IsStandardOutput(config.GetConfigScriptPolicy(), one_of_four, 1, whichType));
    ...
    for (int i = 0; i < 6; i++)
        BOOST_CHECK(!::IsStandardOutput(config.GetConfigScriptPolicy(), malformed[i], 1, whichType));
}
