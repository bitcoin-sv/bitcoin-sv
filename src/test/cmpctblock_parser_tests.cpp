...
constexpr size_t header_len{88};
constexpr size_t nonce_len{8};
constexpr size_t n_len{3}; // 2000 = 0xfd, 0xd0, 0x7 
constexpr size_t sid_len{6};
constexpr size_t sids_per_seg{1'000};
...
const std::vector<uint8_t> cmpctblock_msg{[]
{
    ...
    v.push_back(0xd0);
    v.push_back(0x7);
    v.insert(v.end(), sid_len * sids_per_seg, 8);   // short id
    v.insert(v.end(), sid_len * sids_per_seg, 9);   // short id
...
BOOST_AUTO_TEST_CASE(parse_as_reqd)
{
    ...
    {
        span s{cmpctblock_msg.data() + offset, n}; // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        const auto [bytes_read, bytes_reqd] = parser(s);
        ++passes;
        if(bytes_read)
        {
    ...
    BOOST_CHECK_EQUAL(2'284U, passes);
    BOOST_CHECK_EQUAL(cmpctblock_msg.size(), parser.size());
}
...
BOOST_AUTO_TEST_CASE(parse_byte_by_byte)
{
    ...
    {
        std::span s{cmpctblock_msg.data() + i, 1}; // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        parser(s);
    }
...
BOOST_AUTO_TEST_CASE(read_partial_segment_count)
{
    cmpctblock_parser parser;
    constexpr auto n_bytes{header_len + 1 }; // <- only partial count
    std::ignore = parser(span{cmpctblock_msg.data(), n_bytes});
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(cmpctblock_msg.cbegin(), 
                                  cmpctblock_msg.cbegin() + exp_bytes, 
                                  out.cbegin(), 
...
{
    cmpctblock_parser parser;
    constexpr auto n_bytes{header_len + n_len + sid_len};
    std::ignore = parser(span{cmpctblock_msg.data(), n_bytes});

    vector<uint8_t> out(cmpctblock_msg.size());
    const size_t bytes_read = parser.read(0, span{out.data(), out.size()});
    const auto exp_bytes{n_bytes - sid_len};
    BOOST_CHECK_EQUAL(exp_bytes, bytes_read);
    BOOST_CHECK_EQUAL_COLLECTIONS(cmpctblock_msg.cbegin(), 
                                  cmpctblock_msg.cbegin() + exp_bytes, 
                                  out.cbegin(), 
...
{
    cmpctblock_parser parser;
    constexpr auto n_bytes{header_len + n_len + sid_len * sids_per_seg + sid_len};
    std::ignore = parser(span{cmpctblock_msg.data(), n_bytes});
...
BOOST_AUTO_TEST_CASE(read_partial_prefilled_tx_count)
{
    cmpctblock_parser parser;
    constexpr auto n_segs{2};
    constexpr auto n_bytes{header_len + 
                           n_len + sid_len * sids_per_seg * n_segs +
                           1}; // <- partial prefilledtx len
    std::ignore = parser(span{cmpctblock_msg.data(), n_bytes});
...
BOOST_AUTO_TEST_CASE(read_partial_prefilled_tx_1)
{
    cmpctblock_parser parser;
    constexpr auto n_segs{2};
    constexpr auto n_bytes{header_len + 
...
BOOST_AUTO_TEST_CASE(read_partial_prefilled_tx_2)
{
    cmpctblock_parser parser;
    constexpr auto n_segs{2};
    constexpr auto pftx_len{63};
    constexpr auto n_bytes{header_len + 
                           n_len + sid_len * sids_per_seg * n_segs +
                           pftx_count_len + pftx_len +
                           1 }; // <- partial prefilled tx
    std::ignore = parser(span{cmpctblock_msg.data(), n_bytes});
