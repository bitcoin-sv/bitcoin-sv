...
BOOST_AUTO_TEST_CASE(test_Get) {
    ...
    BOOST_CHECK(
        AreInputsStandard(
            task::CCancellationSource::Make()->GetToken(),
            testConfig.GetConfigScriptPolicy(),
            CTransaction(t1),
            coins,
            0));
...
BOOST_AUTO_TEST_CASE(test_IsStandard) {
    ...
    std::string reason;
    BOOST_CHECK(IsStandardTx(testConfig.GetConfigScriptPolicy(), CTransaction(t), 1, reason));
    ...
    BOOST_CHECK(IsStandardTx(testConfig.GetConfigScriptPolicy(), CTransaction(t), testConfig.GetGenesisActivationHeight() - 1 , reason));
    BOOST_CHECK(IsStandardTx(testConfig.GetConfigScriptPolicy(), CTransaction(t), testConfig.GetGenesisActivationHeight(), reason));
...
BOOST_AUTO_TEST_CASE(test_IsStandard_NonPushOnlyScriptSig_Chronicle)
{
    LOCK(cs_main);
    CBasicKeyStore keystore;
    CCoinsViewEmpty coinsDummy;
    ...
    t.vin.resize(1);
    t.vin[0].prevout = COutPoint(dummyTransactions[0].GetId(), 1);
    // Non-push-only scriptSig: OP_1 OP_1 OP_ADD OP_DROP followed by dummy signature data
    t.vin[0].scriptSig = CScript() << OP_1 << OP_1 << OP_ADD << OP_DROP << std::vector<uint8_t>(65, 0);
    t.vout.resize(1);
    t.vout[0].nValue = 90 * CENT;
    CKey key;
    key.MakeNewKey(true);
    ...
    // Pre-Chronicle: non-push-only scriptSig is non-standard
    BOOST_CHECK(!IsStandardTx(testConfig.GetConfigScriptPolicy(), CTransaction(t), chronicleHeight - 1, reason));
    BOOST_CHECK_EQUAL(reason, "scriptsig-not-pushonly");

    // Post-Chronicle: non-push-only scriptSig is standard
    BOOST_CHECK(IsStandardTx(testConfig.GetConfigScriptPolicy(), CTransaction(t), chronicleHeight, reason));
    BOOST_CHECK(IsStandardTx(testConfig.GetConfigScriptPolicy(), CTransaction(t), chronicleHeight + 1, reason));

    // Sanity: push-only scriptSig is standard at all heights
...
BOOST_AUTO_TEST_CASE(test_IsStandard_MaxTxSizePolicy)
{
    ...
    BOOST_CHECK(IsStandardTx(config.GetConfigScriptPolicy(), tx_lt_def, genesisActivationHeight - 1, reason));

    reason.clear();
    BOOST_CHECK(!IsStandardTx(config.GetConfigScriptPolicy(), tx_gt_def, genesisActivationHeight - 1, reason));
    ...
    BOOST_CHECK(IsStandardTx(config.GetConfigScriptPolicy(), tx_gt_def, genesisActivationHeight, reason));
}
                                     ...
                                     uint64_t initialScriptSize)
{
    ...
    BOOST_CHECK(IsStandardTx(config.GetConfigScriptPolicy(), CTransaction(t), 1, reason));
