...
BOOST_AUTO_TEST_CASE(test_IsStandard) {
    ...
    Amount nDustThreshold = 3 * Amount(182 * testConfig.GetDustRelayFee().GetFeePerK() / 1000);
...
BOOST_AUTO_TEST_CASE(test_IsDustReturnTransaction) {
    ...
    t.vout[0].scriptPubKey << OP_FALSE << OP_RETURN << protocol_id;
    BOOST_CHECK(IsFreeConsolidationTxn(testConfig, CTransaction(t), coins, 1).value);
...
BOOST_AUTO_TEST_CASE(test_IsConsolidationTransactionStandard) {
    ...
    CreateTestConsolidationTxn(t, testConfig, 2, sumInputScriptPubKeySize / 2, inputTxns, 65);
        BOOST_CHECK(IsFreeConsolidationTxn(testConfig, CTransaction(t), coins, tipHeight).value);
    ...
    CreateTestConsolidationTxn(t, testConfig, 2, sumInputScriptPubKeySize / 2 , inputTxns, 65);
    BOOST_CHECK(!IsFreeConsolidationTxn(testConfig, CTransaction(t), coins, tipHeight - 1).value);
