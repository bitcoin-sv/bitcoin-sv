...
int bsv::bint::to_int() const
{
    const auto min = std::numeric_limits<int>::min();
    const auto max = std::numeric_limits<int>::max();

    assert(*this >= bint(min));
    assert(*this <= bint(max));
    const int64_t i{to_int64_t()};
    const int rv{static_cast<int>(i)};
    assert(rv >= min);
    assert(rv <= max);
    return rv;
}

int bsv::bint::to_uint() const
{
    const auto min = std::numeric_limits<uint>::min();
    const auto max = std::numeric_limits<uint>::max();

    assert(*this >= bint(min));
    assert(*this <= bint(max));
    const uint64_t i{to_uint64_t()};
    const uint rv{static_cast<uint>(i)};
    assert(rv <= max);
    return rv;
}
...
namespace
{
    ...
    {
        return unique_asn1_deleter{BN_to_ASN1_INTEGER(bn, nullptr)};
    }
}

int64_t bsv::bint::to_int64_t() const
{
    ...
    assert(*this >= bint{std::numeric_limits<int64_t>::min()});
    assert(*this <= bint{std::numeric_limits<int64_t>::max()});

    const auto x{to_asn1(value_.get())};
    int64_t rv;
    const auto s = ASN1_INTEGER_get_int64(&rv, x.get());
    assert(s);

    return rv;
}

uint64_t bsv::bint::to_uint64_t() const
{
    const auto x = to_asn1(value_.get());
    uint64_t rv;
    const auto s = ASN1_INTEGER_get_uint64(&rv, x.get());
    assert(s);
