...
Arena::Arena(void* base, size_t size, size_t alignment)
    : base_{static_cast<char*>(base)},
      end_{static_cast<char*>(base) + size},
      alignment_{alignment}
{
    // Start with one free chunk that covers the entire arena
    chunks_free_.emplace(base_, size);
}
...
void Arena::free(void* ptr)
{
    ...
    auto i = chunks_used_.find(static_cast<char*>(ptr));
    if(i == chunks_used_.end())
    {
        throw std::runtime_error("Arena: invalid or double free");
    }
    auto freed = *i;
    chunks_used_.erase(i);

    // Add space to free map, coalescing contiguous chunks
    auto next = chunks_free_.upper_bound(freed.first);
    auto prev = (next == chunks_free_.begin()) ? chunks_free_.end() : std::prev(next);
    if(prev == chunks_free_.end() || !extend(prev, freed))
        prev = chunks_free_.emplace_hint(next, freed);
    if(next != chunks_free_.end() && extend(prev, *next))
        chunks_free_.erase(next);
}

Arena::Stats Arena::stats() const
{
    Arena::Stats r{0, 0, 0, chunks_used_.size(), chunks_free_.size()};
    for(const auto& chunk : chunks_used_)
        r.used += chunk.second;
    for(const auto& chunk : chunks_free_)
        r.free += chunk.second;
...
LockedPool::LockedPool(std::unique_ptr<LockedPageAllocator> allocator,
...
void* LockedPool::alloc(size_t size)
{
    ...
    {
        return arenas_.back().alloc(size);
    }
    return nullptr;
}
...
{
    std::lock_guard<std::mutex> lock{mutex_};
    LockedPool::Stats r{0, 0, 0, cumulative_bytes_locked_, 0, 0};
...
bool LockedPool::new_arena(size_t size, size_t align)
{
    ...
    if(arenas_.empty())
    {
        size_t limit = allocator_->GetLimit();
        if (limit > 0) {
    ...
    else if(lf_cb_)
    {
        ...
        if(!lf_cb_())
        {
            ...
            allocator_->FreeLocked(addr, size);
            return false;
        }
    }
    arenas_.emplace_back(allocator_.get(), addr, size, align);
    return true;
}

LockedPool::LockedPageArena::LockedPageArena(LockedPageAllocator* allocator,
                                             void* base,
                                             size_t size,
                                             size_t align)
    : Arena{base, size, align},
      base_{base},
      size_{size},
...
LockedPool::LockedPageArena::~LockedPageArena() { allocator_->FreeLocked(base_, size_); }
