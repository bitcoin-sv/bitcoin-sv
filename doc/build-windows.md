...
Below are some notes on how to build Bitcoin SV for Windows.

The options known to work for building Bitcoin SV on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Bitcoin SV Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Bitcoin SV applications.
...
Note that for WSL the Bitcoin SV source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoin-sv, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitcoin-sv`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoin-sv
