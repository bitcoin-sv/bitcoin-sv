...
class TriggerSafeModeAfterReorg(BitcoinTestFramework):
    ...
    def run_test(self):
        with self.run_node_with_connections("Preparation", 0, None, 2) as (conn1, conn2):
            ...
            send_by_headers(conn2, branch_2_blocks[:SAFE_MODE_DEFAULT_MIN_VALID_FORK_LENGTH + 3], do_send_blocks=True)

            # active tip is from branch 2 and branch 1 has status valid-fork
            wait_for_tip(conn1, branch_2_blocks[SAFE_MODE_DEFAULT_MIN_VALID_FORK_LENGTH + 2].hash)
            ...
            send_by_headers(conn1, branch_2_blocks[SAFE_MODE_DEFAULT_MIN_VALID_FORK_LENGTH + 3:], do_send_blocks=True)
