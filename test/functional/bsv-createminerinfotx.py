...
class CreateMinerInfoTest(BitcoinTestFramework):
    ...
    def one_test(self, allKeys, fundingSeedTx, test_case):
        ...
        if test_case == self.TEST_call_replace:
            # for now we need to add a new fund when replacing
            self.store_funding_info(0, allKeys.fundingKeys, fundingSeedTx.hash, index=1)
        ...
        sleep(1) # to give time for relaying
