...
"""
This test checks activation of magnetic opcodes
"""
...
class MagneticActivationTest(ComparisonTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [['-whitelist=127.0.0.1',
                            ...
                            "-replayprotectionactivationtime=%d" % (2 * MAGNETIC_START_TIME)]]
    ...
    def create_and_tx(self, count):
        ...
        for _ in range(count):
            tx.vout.append(CTxOut(value, CScript([OP_1, OP_1, OP_MUL])))
    ...
    def get_tests(self):
        ...
        # Create various outputs using the OP_MUL to check for activation.
        ...
        tx.rehash()
        spendable = [PreviousSpendableOutput(
        ...
        self.log.info("Try to use the magnetic opcodes before activation")
        ...
        node.setmocktime(MAGNETIC_START_TIME)
        ...
        self.log.info("Cause a reorg that deactivate the magnetic opcodes")

        # Invalidate the magnetic block, ensure tx0 gets back to the mempool.
        assert(tx0id not in set(node.getrawmempool()))

        node.invalidateblock(format(magneticblock.sha256, 'x'))
        assert(tx0id in set(node.getrawmempool()))
