...
"""
Check -acceptp2sh=0 flag which rejects transactions with P2SH in output after Genesis.
Check -acceptp2sh=1 flag which treats transactions with P2SH as non-standard. 
"""
...
class BSVDiscourageP2SH(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        with self.run_node_with_connections("reject P2SH", 0, ['-whitelist=127.0.0.1', '-acceptnonstdtxn=0', '-acceptp2sh=0', '-genesisactivationheight=100'], 1) as connections:
        ...
        with self.run_node_with_connections("treat P2SH as non-standard", 0, ['-whitelist=127.0.0.1', '-acceptnonstdtxn=0', '-acceptp2sh=1', '-genesisactivationheight=100'], 1) as connections:
            p2sh_tx = self.makeP2SHTransaction()
            self.assert_rejected_transaction(p2sh_tx, connections[0].cb, b'scriptpubkey')
            # check that rejected transaction did not cause disconnection
            connections[0].cb.sync_with_ping()

        with self.run_node_with_connections("accept P2SH as non-standard", 0, ['-whitelist=127.0.0.1', '-acceptnonstdtxn=1', '-acceptp2sh=1', '-genesisactivationheight=100'], 1) as connections:
            p2sh_tx = self.makeP2SHTransaction()
